  ...
  return (
    <div className="fixed top-0 left-0 w-full max-w-full z-50 ">
        <div className="flex items-center my-6 px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
          <a className="block w-[12rem] xl:mr-8 text-color-2 font-titan" href="./index.html">42Baking</a>
          ...
          <div className="flex ">
            {loginNav.map((item) => (
                <button
                  ...
                  href={item.url}
                  onClick={handleClick}
                  className={`block relative py-2 px-6 mx-2 lg:-mr-0.25  text-color-6 font-poppins font-bold bg-color-4 button `}>
                  {item.title}
                </button>
              ))}
          </div>
